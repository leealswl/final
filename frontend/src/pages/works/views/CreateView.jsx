...
const CreateView = () => {
    ...
    const tocSections = useMemo(() => tableOfContents?.sections ?? [], [tableOfContents]);
    ...
    if (!analysisResult) {
        return (
            <Stack justifyContent="center" alignItems="center" height="100vh" sx={{ background: 'linear-gradient(135deg, #f5f7fa 0%, #e4ecf7 100%)' }} spacing={2}>
                <Typography fontSize="1.6rem" fontFamily="Isamanru-Bold">
                    아직 분석 데이터가 없습니다.
                </Typography>
                <Typography color="#8C8C8C">먼저 분석을 실행한 뒤 생성 페이지로 이동해주세요.</Typography>
            ...
            <Stack spacing={4} mx="auto" maxWidth="1200px">
                ...
                <Paper elevation={0} sx={{ p: 4, borderRadius: 3, background: 'linear-gradient(135deg, #111827, #1f2937)', color: 'white' }}>
                    ...
                    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} mt={3}>
                        ...
                        />
                        {userForm?.source_file && <Chip label={`출처 파일: ${userForm.source_file}`} sx={{ backgroundColor: 'rgba(255,255,255,0.12)', color: 'white', fontWeight: 500 }} />}
                        {!userForm?.source_file && tableOfContents?.source_file && (
                            <Chip label={`목차 출처: ${tableOfContents.source_file}`} sx={{ backgroundColor: 'rgba(255,255,255,0.12)', color: 'white', fontWeight: 500 }} />
                        )}
                    </Stack>
                </Paper>
                                    ...
                                    <Grid item xs={12} md={6} key={key}>
                                        <Stack spacing={1.2}>
                                            <Typography fontWeight={600} fontSize="1rem">
                                                ...
                                                {field.required && (
                                                    <Typography component="span" color="#ff4d4f">
                                                        {' '}
                                                        *
                                                    </Typography>
                                                )}
                                            ...
                                            <Stack direction="row" spacing={1}>
                                                <Chip label={field.field_type || 'text'} size="small" sx={{ backgroundColor: '#F0F5FF', color: '#1d39c4' }} />
                                                {field.source && <Chip label={`출처: ${field.source}`} size="small" sx={{ backgroundColor: '#F6FFED', color: '#389e0d' }} />}
                                            </Stack>
                                        </Stack>
                                    </Grid>
                                );
                        ...
                        >
                            <Stack spacing={1.5}>
                                ...
                                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}>
                                    <Chip label="폼 유형: 목차 기반" sx={{ backgroundColor: 'rgba(255,255,255,0.12)', color: 'white', fontWeight: 500 }} />
                                    ...
                                    <Stack spacing={2.5} p={{ xs: 3, md: 4 }}>
                                        <Stack spacing={0.75}>
                                            <Typography fontSize="1.2rem" fontWeight={700} color="#1f3b73">
                                                ...
                                                {field.required && (
                                                    <Typography component="span" color="#d4380d">
                                                        {' '}
                                                        *
                                                    </Typography>
                                                )}
                                            ...
                                            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                                                ...
                                                />
                                                <Chip label="응답 형식: 장문 텍스트" size="small" sx={{ backgroundColor: '#F0F5FF', color: '#1d39c4' }} />
                                            </Stack>
                                        </Stack>
                                ...
                                <Paper key={section.number} variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                                    <Stack spacing={0.5}>
                                        ...
                                        <Stack direction="row" spacing={1}>
                                            ...
                                            />
                                            {tableOfContents.source && <Chip label={`출처: ${tableOfContents.source}`} size="small" sx={{ backgroundColor: '#E6F4FF', color: '#0958d9' }} />}
                                        </Stack>
                                    </Stack>
                                </Paper>
                            ))}
                ...
                <Paper elevation={0} sx={{ p: 4, borderRadius: 3, backgroundColor: 'white' }}>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} md={4}>
                            <Stack spacing={0.5}>
                                <Typography color="#8C8C8C" fontSize="0.85rem">
                                    분석 상태
                                </Typography>
                        ...
                        <Grid item xs={12} md={4}>
                            <Stack spacing={0.5}>
                                ...
                                <Typography fontWeight={600}>{analysisResult.data?.extracted_at || analysisResult.data?.timestamp || '-'}</Typography>
                            </Stack>
                        </Grid>
                    </Grid>
                    ...
                    <Typography fontSize="0.85rem" color="#8C8C8C">
                        🔒 작성한 데이터는 아직 서버에 저장되지 않았습니다. 임시 저장 버튼을 활용하여 초안을 확인하고, 필요 시 차후 API 연동을 통해 제출 프로세스를 연결하세요.
                    </Typography>
                </Paper>
