...
import { Box, Stack, Typography, Button, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Menu, MenuItem } from '@mui/material';
...
const FileIcon = ({ mime = '', name = '' }) => {
    ...
    if (mime.startsWith('text/') || mime.includes('markdown') || lower.endsWith('.md') || lower.endsWith('.txt')) return <DescriptionIcon fontSize="small" sx={{ mr: 1 }} />;
...
const resolveRootId = (selectedNode, tree) => (selectedNode?.type === 'folder' && /^root-\d{2}$/.test(selectedNode.id) ? selectedNode.id : tree?.[0]?.id ?? 'root-01');
...
export default function Sidebar() {
    ...
    const renderNode = (node) => {
        ...
        const label = (
            <Stack direction="row" alignItems="center" spacing={0.5} sx={{ pr: 1, '&:hover .row-actions': { opacity: 1 } }} onContextMenu={(e) => openMenu(e, node)}>
                {isFolder ? <FolderIcon fontSize="small" sx={{ mr: 1, color: '#6b7280' }} /> : <FileIcon mime={node.mime} name={node.name} />}
    ...
    return (
        ...
        /> */}
            ...
            <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
                ...
                <DialogContent>
                    <Typography variant="body2">{deleteTarget?.name}을(를) 삭제할까요?</Typography>
                </DialogContent>
                <DialogActions>
