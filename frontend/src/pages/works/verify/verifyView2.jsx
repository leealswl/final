...
function LawVerifyDashboard({ results }) {
  ...
  const {
    ...
    const sortedEntries = entries.sort(([, a], [, b]) => {
      ...
      const statusDiff =
        (STATUS_ORDER[bStatus] || 0) - (STATUS_ORDER[aStatus] || 0);
          ...
          >
            ...
            >
              {statusChartData.length === 0 ? (
                <Typography
                  variant="body2"
                  sx={{ color: 'text.secondary', textAlign: 'center', mt: 7 }}
                >
                  아직 검증 결과가 없습니다.
                </Typography>
              ) : (
                        ...
                        >
                          {statusChartData.map((entry, index) => (
                            <Cell
                              ...
                              fill={STATUS_COLORS[entry.name] || '#90a4ae'}
                            />
                          ))}
                        </Pie>
                        <Tooltip />
      ...
      settled.forEach((res, idx) => {
        ...
        if (res.status === 'fulfilled') {
          ...
          next[f.key] = {
            ...
            risk_level: 'UNKNOWN',
            reason:
              '이 관점 검증 중 오류가 발생했습니다. 나중에 다시 시도해 주세요.',
          };
        }
      });
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            현재 기획서 초안을 기반으로 예산, 수행체계, 성과관리 등 주요 관점에서
            국가 R&D 관련 법령·지침 준수 여부를 자동으로 점검합니다.
          </Typography>
        </Box>
      ...
      {!text && (
        <Box sx={{ mt: 1 }}>
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            초안 텍스트를 불러오는 중이거나 아직 생성되지 않았습니다. 생성/작성
            단계에서 기획서를 저장한 후 다시 검증을 실행해 주세요.
          </Typography>
        </Box>
      )}
