    ...
    if (hasLaw) {
      Object.entries(results).forEach(([_, r]) => {
        ...
        if (hasCompare) {
            ...
            const normalizedMissingFeatures = buildNormalizedMissingFeatureList(rawMissingFeatures);
        ...
        <Card sx={{ mt: 3 }}>
            <CardContent>
                <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
                    ...
                    <Chip icon={<WarningAmberIcon />} label={`${items.length}개 핵심 보완 포인트`} size="small" color="warning" />
                </Stack>

                <Typography sx={{ color: 'text.secondary', mb: 2, fontSize: 14 }}>아래 항목부터 순서대로 보완하면, 심사 관점에서 눈에 띄는 리스크를 빠르게 줄일 수 있습니다.</Typography>
        ...
        <Card sx={{ mt: 4 }}>
            <CardContent>
                ...
                <Typography sx={{ color: 'text.secondary', mb: 2, fontSize: 14 }}>
                    주요 관점별로 어떤 이유로 적합/보완/부적합 판정이 났는지, 법령 위반 가능성이 어디에서 발생하는지 확인할 수 있습니다.
                </Typography>
                    ...
                    <Accordion key={key || idx} sx={{ boxShadow: 'none' }}>
                        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                            <Stack direction="row" spacing={1} alignItems="center">
                                ...
                                {r.violation_judgment && <Chip size="small" variant="outlined" label={JUDGMENT_LABELS[r.violation_judgment] || r.violation_judgment} />}
                            </Stack>
                        </AccordionSummary>
                      ...
                      <ListItem key={i} alignItems="flex-start">
                        <ListItemText
                          ...
                          secondary={
                            ...
                            {r.missing?.length > 0 && (
                                <Box sx={{ mb: 2 }}>
                                    <Typography sx={{ fontWeight: 600, mb: 0.5 }}>부족한 요소</Typography>
                                    <List dense>
                                        {r.missing.map((m, i) => (
                                            <ListItem key={i}>
                                                <ListItemText primary={m} />
                            ...
                            {r.suggestion && (
                                <Box sx={{ mb: 2 }}>
                                    <Typography sx={{ fontWeight: 600, mb: 0.5 }}>보완 제안</Typography>
                            ...
                            {r.violations?.length > 0 && (
                                <Box sx={{ mb: 2 }}>
                                    <Typography sx={{ fontWeight: 600, mb: 0.5 }}>법령 위반 가능성이 있는 조항</Typography>
                                    <List dense>
                                        {r.violations.map((v, i) => (
                                            <ListItem key={i} alignItems="flex-start">
                                                <ListItemText
                                                    primary={
                                                        <Typography variant="body2">
                                                            {v.law_name} {v.article_no ? `${v.article_no} ` : ''}
                                                            {v.article_title}
                                                        </Typography>
                                                    }
                                                    secondary={
                            ...
                            {r.related_laws?.length > 0 && (
                                <Box>
                                    <Typography sx={{ fontWeight: 600, mb: 0.5 }}>참고할 법령</Typography>
        ...
        <Typography sx={{ color: "text.secondary", mb: 2, fontSize: 14 }}>
          ...
          단순 안내 정보는 리포트에서 제외됩니다.)
                ...
                {sectionDetails.length > 0 && (
                    <>
                        <Typography sx={{ fontWeight: 600, mt: 1, mb: 1 }}>섹션별 상세 분석</Typography>
                ...
                {featureDetails.length > 0 && (
                    <>
                        <Divider sx={{ my: 2 }} />
                        <Typography sx={{ fontWeight: 600, mb: 1 }}>세부 조건별 분석 (지원대상, 사업기간, 예산 등)</Typography>
                        <Typography sx={{ color: 'text.secondary', mb: 1.5, fontSize: 13 }}>
                            공고문에서 추출한 세부 조건(지원대상, 사업기간, 예산 조건 등)을 기준으로, 초안의 표현이 충분한지/조건을 정확히 맞추고 있는지 점검한 결과입니다. 문의처·공고기관·접수기관 등
                            단순 안내 정보는 여기에서 제외됩니다.
                        </Typography>
                ...
                {missingSections.length === 0 && missingFeatures.length === 0 && sectionDetails.length === 0 && featureDetails.length === 0 && (
                    <Box sx={{ mt: 1 }}>
                        <Typography sx={{ color: 'text.secondary' }}>공고문 기준으로 분석할 세부 항목이 없습니다.</Typography>
                    </Box>
                )}
            </CardContent>
        </Card>
          ...
          <Typography sx={{ color: "text.secondary", fontSize: 14 }}>
            ...
            <b>공고문-초안 비교 결과</b>를 한눈에 정리한 리포트입니다.
                ...
                <Stack direction="row" spacing={1.5}>
                    <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/works/verify')}>
                        검증 화면으로 돌아가기
                    </Button>
                </Stack>
            </Stack>
