...
async def generate_content(request: ChatRequest):
    try:
        print("📢 기획서 생성 요청 수신:", request.userMessage)
        ...
        # 💡 thread_id는 고정 ("test") - 계속 같은 세션 유지
        thread_id = "test"
        print(f"🔄 기존 세션 사용: {thread_id}")

        # --- 2. AsyncSqliteSaver로 기존 상태 확인 ---
        async with AsyncSqliteSaver.from_conn_string(DB_PATH) as saver:
            proposal_app = proposal_graph.compile(checkpointer=saver)
            ...
            try:
                ...
                print(f"📦 기존 상태 발견:")
                print(f"  - 완료된 목차: {existing_state.values.get('completed_chapters', [])}")
                print(f"  - 현재 목차: {existing_state.values.get('target_chapter', '')}")
                print(f"  - 대기 중 질문: {len(existing_state.values.get('pending_questions', []))}개")
                print(f"  - 답변된 질문: {len(existing_state.values.get('answered_questions', []))}개")
            except Exception as e:
                print(f"⚠️ 기존 상태 없음 (첫 요청): {e}")
                existing_state = None
            
            # --- 3. initial_state 구성 (기존 상태 유지) ---
            initial_state = {
                "user_id": str(request.userIdx) if request.userIdx else "unknown",
                "project_idx": request.projectIdx,
                "user_prompt": request.userMessage,  # ✅ 새 메시지만 추가
                
                "fetched_context": context_data,
                
                # ✅ 다음 필드들은 초기화하지 않음 (LangGraph가 자동 복원)
                # "pending_questions": [],  ❌ 제거!
                # "answered_questions": [], ❌ 제거!
                # "completed_chapters": [], ❌ 제거!
                # "target_chapter": "",     ❌ 제거!
                # "guide": [],              ❌ 제거!
                # "draft_toc_structure": [], ❌ 제거!
                ...
                # ✅ messages는 load_data 노드에서 처리됨
                # "domain"은 필요시 외부에서 설정
            }
            ...
            print(f"완료된 목차: {final_state.values.get('completed_chapters', [])}")
            print(f"현재 목차: {final_state.values.get('target_chapter', '')}")
            ...
            print(f"전체 질문: {len(pending_q)}개")
            print(f"답변 완료: {len(answered_q)}개")
            print(f"남은 질문: {len(remaining)}개")
            print("--- 현재 상태 확인 ---\n")
