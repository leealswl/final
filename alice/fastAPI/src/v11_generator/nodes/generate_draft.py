...
def generate_proposal_draft(state: ProposalGenerationState) -> ProposalGenerationState:
    ...
    # DRAFT_PROMPT = """
    #     당신은 한국 정부 RFP(제안요청서)·입찰·지원사업 제안서 작성 전문가이며,
    #     평가 위원이 단시간에 핵심을 파악할 수 있도록 **완벽한 개조식 문체**를 구사합니다.
    ...
    #     ✍️ <작성 지침>
    #     - 위 네 가지 입력 정보를 모두 반영하여 **정부 제안서 표준 양식의 완성된 단락**을 작성하십시오.
    #     - **제안서 작성 가이드**에 명시된 해당 목차의 작성 요령, 핵심 포인트, 필수 포함 내용을 반드시 준수하십시오.
    ...
    #        - 예: "효과가 기대됩니다." (X) → "**기대 효과 창출**" 또는 "**효과 전망**" (O)
    #     2. **금지 사항:**
    #        - '~합니다', '~입니다', '~해요', '~할 수 있습니다' 등의 **구어체/경어체/평서형 어미 사용을 절대 금지**합니다.
    #        - '저희', '우리' 등의 1인칭 대명사를 사용하지 말고 '동사', '당사'로 표기하거나 주어를 생략하십시오.
    #     3. **간결성 및 명확성:**
    #        - 불필요한 조사(은/는/이/가)와 접속사를 최대한 배제하고 핵심 키워드 위주로 작성하십시오.
    #        - 추측성 표현(~일 것 같음) 대신 단정적이고 확신에 찬 표현(~임, ~확보, ~달성)을 사용하십시오.
    ...
    #           "type": "paragraph",
    #           "attrs": {{ "textAlign": "left", "paragraphIndex": 0 }},
    #           "content": [{{ "type": "text", "text": "핵심 내용 요약 (개조식)" }}]
    #         }},
    #         {{
    #           "type": "paragraph",
    #           "attrs": {{ "textAlign": "left", "paragraphIndex": 1 }},
    ...
    #           "type": "paragraph",
    #           "attrs": {{ "textAlign": "left", "paragraphIndex": 0 }},
    #           "content": [{{ "type": "text", "text": "표 설명 및 핵심 요약" }}]
    #         }},
    #         {{
    #           "type": "table",
    #           "attrs": {{ "class": "paladoc-table" }},
    ...
    #                 {{
    #                   "type": "tableCell",
    #                   "content": [{{ "type": "paragraph", "content": [{{ "type": "text", "text": "추진 목표" }}] }}]
    #                 }},
    #                 {{
    #                   "type": "tableCell",
    #                   "content": [{{ "type": "paragraph", "content": [{{ "type": "text", "text": "데이터 기반 서비스 구축" }}] }}]
    #                 }}
    #               ]
    #             }}
    #           ]
    ...
    #           "type": "paragraph",
    #           "attrs": {{ "textAlign": "left", "paragraphIndex": 0 }},
    #           "content": [{{ "type": "text", "text": "차트 분석 내용" }}]
    #         }},
    #         {{
    #           "type": "chart",
    #           "attrs": {{
    ...
    print('-' * 50)
    print('anal_guide_summary: ', anal_guide_summary)
    print('-' * 50)
    ...
    print('-' * 50)
    print('guide_reference: ', guide_reference)
    print('-' * 50)
